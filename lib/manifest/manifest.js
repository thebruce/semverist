...
module.exports = function manifestFactory(pluginName, configNameSpace, config) {
  ...
  if (manifestConfig.composer.composerType === 'default') {
    ...
    if (manifestConfig.composer.composerType === 'json') {
      // use directoryConverter and directory classes
      configDeducedPlugins.push(plugins.manifest.jsonWriter);
  ...
  return setUp.then(extendedClass => class manifest extends extendedClass {
    ...
    static createConverter(converterSource) {
      ...
      let converter;
      if (staticConfig.converterType === 'directory' || staticConfig.converterType === 'json') {
        // use directoryConverter and directory classes
        converter = converterFactory('semverist', `${staticConfig.converterType}Converter`)
          .then((ConverterClass) => {
