...
module.exports = function manifestFactory(
  pluginName,
  configNameSpace,
  config,
  ...
  if (pluginName) {
    ...
    // If not we will only add our deduced plugins based on config.
    const superClasses = molotov
      .mixSupers()
      .getMolotov()
      .getSupers();
    setUp = configDeducedPlugins.reduce(
  ...
  return Promise.resolve(setUp).then(
    extendedClass =>
      class manifest extends extendedClass {
        /**
