...
module.exports = class semverishSuper extends semveristSuperBase {
  ...
  init(semverishValue, configs) {
    ...
    this.setSemverishArray(this.getSemverish());
    super.init(configs);
  }
  ...
  setSemveristElement(semverishValue) {
    ...
    // Set the raw element incase we have a deep path.
  ...
  extractSemverishFromPath(semverishValue) {
    try {
      ...
      if (tmpArray.length > 1) {
        let replacer = `${tmpArray[0]}`;
        replacer = tmpArray.length >= 2 ? `${replacer}.${tmpArray[1]}` : replacer;
        // We have the possibility of a prerelease.
        ...
        if ((element !== '') && !_.isFinite(Number(element))) {
          ...
          if (elementArray.length > 1) {
            if (_.isFinite(_.parseInt(elementArray[0]))) {
              elementArray.shift();
            }
      ...
      throw new Error('The semverish value must be able to be converted to a semver value.');
    }
  }
