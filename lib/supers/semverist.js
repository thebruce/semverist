 ...
 * @alias module:semveristSuper
 */
const semverist = class semverSuperBase {
  /**
   * Creates an instance of semverSuperBase.
   */
  constructor() {
  ...
  init(configs) {
    ...
    const semveristDefaultConfig = defaultConfig;
    ...
    const configWithTemp = Object.assign({}, semveristDefaultConfig, tmpConfig);

    // Now set needed defaults.
    this.setDefaultName(configWithTemp.semveristBehaviors.defaultName);

    this.setSemveristGroups(configWithTemp.groups);

    this.setSemveristConfig(configWithTemp);
  }
   ...
   */
  splitStringNoEmpties(semverishString) {
   ...
   *
   * @returns {string}
   *   The default name of this semverist. Typically "default".
   */
  getDefaultName() {
  ...
  /**
   * Translates levels between semver and semver utils
   *
   * @static
   * @param {string} level
   *  the level of semver
   * @returns {number}
   *   THe index of the indicated level.
   */
  static getSemverLevelIndex(level) {
    let tmpLevel = level;
    if (tmpLevel === 'prerelease') {
      tmpLevel = 'release';
    }
    return this.getSemverLevels().indexOf(tmpLevel);
  }
   ...
   *
   * @returns {RegExp}
   *   Returns the regular expression for a prelease pattern.
   */
  ...
  doesAttributeBelongToGroup(attributeName, groupName) {
    if (!_.has(this.getSemveristGroups()[groupName], 'members')) {
      throw new Error(
        `${groupName} group is not properly formed and has no members array`
      );
    }
    if (
