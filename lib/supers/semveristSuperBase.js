...
const SemveristSuperBase = class semverSuperBase {
  ...
  createOptions(attribute, inheritenceOverride) {
    ...
    else if (inheritenceOverride !== undefined) {
      options.inheritence = inheritenceOverride;
    }
    else if (_.has(this.getSemveristConfig().semveristBehaviors, 'inheritence')) {
  ...
  setPreReleasePattern(pattern) {
    if (pattern !== undefined) {
      ...
      this.preReleasePattern = /\d+-([0-9A-Za-z-]*)(\.\d*(\+[a-zA-Z0-9-]*)*\.?|\+[a-zA-Z0-9-]+\.?)?/g; // eslint-disable-line max-len
    }
  }
