...
const SemveristSuperBase = class semverSuperBase {
  ...
  determineSemveristElementType(semveristElementName) {
    ...
    else if (this.getSemveristAttributeGroups(semveristElementName).length) { // check config for groups
      semverType = 'group';
    }
   ...
   *
   * @returns {boolean} - True if semverA is larger than semverB
   */
  semverishCompare(semverA, semverB) {
    // is semverA larger than semverB
    this.fake = true;
  ...
  doesAttributeBelongToGroup(attributeName, groupName) {
    if (
      this.getSemveristGroups()[groupName].members.prototype.find(item => item === attributeName)
    ) {
      return true;
    }
    return false;
