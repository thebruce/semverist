...
const SemveristSuperBase = class semverSuperBase {
  ...
  addSemverishToSemverRange(value, semverRange, withInheritence, withLazySemver) {
     ...
     * TODO: So we probably need a path to range function that takes
   ...
   * @param {any} options
   */
  pathToRange(path, options) {
    // Single Values.
    // If path is 3 parts long and no part is prerelease -
    //   this is a single item, regardless.
    // if path is 4 parts long and part of prerelease -
    //   this is a single item, regardless.

    // inheritence
    // If a path is only 2 parts or 1 parts it is ~x.y or ~x
    // if we have 3 and prerelease  ~x.y.z

    // inheritence and lazy semver
    // If we have 1 ~x
    //
    // If we have 2 x.y and x is not 0 ^x.y.0 if x = 0 ~0
    //
    // if we have 3 or more x.y.z and x is not 0 and
    // no pre x ^x.y.z, if x is 0 but not y then > 0.y.z < 1.0.0,
    // if x & y are 0 then > 0.0.z < 1.0.0
   ...
   */
  createOptions(attribute, inheritenceOverride, lazySemverOverride) {
    const options = {
      attributeType: attribute,
      inheritence: this.getSemveristConfig().semveristBehaviors.inheritence,
      lazySemverist: false
    };
    if (
      (attribute !== 'semveristObject')
      &&
      (_.has(this.getSemveristConfig().semveristBehaviors.lazySemverist, attribute))
      &&
      this.getSemveristConfig().semveristBehaviors.lazySemverist[attribute]
    ) {
      options.lazySemverist = true;
    }
    ...
    if (lazySemverOverride !== undefined) {
      options.lazySemverist = lazySemverOverride;
    }
