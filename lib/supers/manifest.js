...
const manifest = class manifestSuperBase {
  ...
  addItemAlters(semverValue, converterItem) {
    this.bumpkin = semverValue;
    this.lintingIsFun = converterItem;
    // We should have item alters that in consideration
    // Check the strategy to see what if any alters we should keep.

    // Once we know what applies we should add these alters in order
    // with the original semverish value for this semver realization
    // so that they merge or replace correctly.
  }
