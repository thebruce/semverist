  ...
  setManifestComponents() {
    ...
    semverRealizations.forEach(semverItem => {
      ...
      attributeKeys.forEach(attKey => {
        ...
        const attributeRangeKeys = Object.keys(
          this.getConverter().attribute[attKey]
        );
        const isAttributeActualized = attributeRangeKeys.filter(keyInQuestion =>
          semver.satisfies(semverItem, keyInQuestion)
        );
