...
const manifest = class manifestSuperBase {
  ...
  getElementPathExtension(pather) {
    ...
    if (
      this.getConfig().converterType !== 'default'
      &&
        this.getConfig().composer.utilizeExtantExtensions) {
      const last = pather.split('.')[pather.split('.').length - 1];
      const extantRanges = Object.keys(this.getConverter().attribute[last]);

      extantRanges.forEach((range) => {
        if (semver.satisfies(_.initial(pather.split('.')).join('.'), range)) {
          if (this.getConverter().attribute[last][range].extension) {
            tmpExtension = this.getConverter().attribute[last][range].extension;
          }
        }
      });
    }
    else {
      tmpExtension = this.getConfig().composer.defaultExtension;
    }
    return tmpExtension;
  }
