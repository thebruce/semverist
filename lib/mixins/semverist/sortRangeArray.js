...
module.exports = superclass => class extends superclass {
  ...
  sortSemverishArray(semverishArray) {
    ...
    if (semverishArray && _.isArray(semverishArray)) {
      ...
      throw new Error('Can not sort a semverish Array that is not actually range.');
    }
  ...
  static semverishSort(tmpObject) {
    ...
    if (keyCount !== _.uniq(_.values(tmpObject)).length) {
      ...
      if (sentinelTruth) {
        ...
        const count = sortedCount => sortedCount.reduce((a, b) => Object.assign(a, { [b]: (a[b] || 0) + 1 }), {}); // eslint-disable-line max-len
        ...
        dupes.forEach((value) => {
          ...
          // Sort those items
          const keySort = _.orderBy(allOccurences, newValue => newValue.length, 'asc');
