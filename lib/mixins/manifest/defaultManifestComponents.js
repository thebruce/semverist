  ...
  class extends superclass {
    ...
    addItemAlters(semverValue, converterItem) {
      ...
      if (Object.keys(converterDefaults).length > 0) {
        ...
        const filterApplies = defaults.filter(defaultRange =>
          semver.satisfies(semverValue, defaultRange)
        );
        ...
        if (filterApplies.length) {
          ...
          this.addManifestComponentItem(
            ...
            semverValue,
            `${
              this.getConverterDefaults()[defaultKey][filterApplies[0]]
                .semverishValue
            }.${defaultKey}` // eslint-disable-line max-len
          );
        }
      }
