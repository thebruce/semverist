...
module.exports = superclass => class extends superclass {
  ...
  prioritizeComponent(componentPathArray) {
    ...
    let defaultArray = [];
    const groupArray = [];
    const { components } = this.getManifestComponents()[elementName][semverValue];
    ...
    if (capabilities.indexOf('default') !== -1) {
      ...
      defaultArray = defaultArray.concat(components.filter(item => item.indexOf(`.${defaultName}`) !== -1)); // eslint-disable-line max-len
    }
