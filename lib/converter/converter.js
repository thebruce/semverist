...
module.exports = function converterFactory(
  superNameSpace,
  pluginName,
  ...
  return Promise.resolve(setUp).then(
    extendedClass =>
      class converter extends extendedClass {
        /**
        ...
        setSemverishObject(semverishObject) {
          ...
          if (super.setSemverishObject)
            super.setSemverishObject(semverishObject);
        }
        ...
        instantiateSemverish(semverish, levelPrep, key) {
          ...
          if (semverish.getSemverParsed().release) {
            ...
            // configurable.
            if (
              this.splitStringNoEmpties(semverish.getSemverParsed().release)
                .length === 1
            ) {
        ...
        semveristAssemble(level) {
          ...
          if (level === 'root') {
            ...
            semveristAtLevel = Object.keys(
              _.get(this.getSemverishObject(), level)
            );
            levelPrep = `${level}.`;
          }
          ...
          semveristAtLevel.forEach(key => {
            ...
            // value.
            const semverishClass = semverishFactory(
              'semverist',
              'semverish',
              molotovConfig
            ).then(SemverishClass => new SemverishClass());
            const rangeClass = rangeFactory(
              'semverist',
              'range',
              molotovConfig
            ...
            const semveristClass = Promise.all([
              semverishClass,
              rangeClass,
            ]).then(semveristClassArray => {
              // Both factories are ready to produce full fledged classes
              // for this key.
              const semverish = this.instantiateSemverish(
                semveristClassArray[0],
                levelPrep,
                key
              );
              const range = this.instantiateRange(
                semveristClassArray[1],
                _.cloneDeep(semverish)
              );
          ...
          return Promise.all(promises).then(() => this.getSemveristClasses());
        }
         ...
         */
        createConverter() {
          return this.semveristAssemble('root').then(() => this.toConverter());
        }
            ...
            Object.keys(tmpClasses[typeKey]).forEach(elementKey => {
              ...
              semverishKeys.forEach((semverishKey, index, collection) => {
                ...
                const children = _.filter(
                  ...
                  thing =>
                    this.analyzeSemverishAdjacency(semverishKey, thing)
                      .child === true
                );
                ...
                // as exceptions.
                this.sortSemverishArray(children).forEach(
                  semverishException => {
                ...
                if (
                  ...
                  if (collection.length > index + 1) {
                    range.adjustTerminalBounds(
                      tmpClasses[typeKey][elementKey][
                        collection[index + 1]
                      ].range.getSemverish()
                    ); // eslint-disable-line max-len
                  }
                  ...
                  this.getSemveristConfig().semveristBehaviors.inheritence ===
                    'semverImplied' &&
                  semver.parse(this.valueToSemver(semverishKey)).prerelease
                    .length > 0
                ) {
                  ...
                  if (adjacent.length) {
                    range.setTerminalBounds(
                      this.sortSemverishArray(adjacent)[0]
                    );
                    tmpAdjacent = true;
                  }
                  ...
                  if (!tmpAdjacent) {
                    ...
                    const parsed = semver.parse(
                      this.valueToSemver(semverishKey)
                    );
                    const currentName = parsed.prerelease[0];
                    const majMinPatch = `${parsed.major}.${parsed.minor}.${
                      parsed.patch
                    }`;
                    const preReleaseRelatives = _.without(
                      _.filter(semverishKeys, key => {
                        const newParsed = semver.parse(this.valueToSemver(key));
                        return (
                          newParsed.prerelease.length > 0 &&
                          `${newParsed.major}.${newParsed.minor}.${
                            newParsed.patch
                          }` === majMinPatch
                        );
                      }),
                    ...
                    if (preReleaseRelatives.length > 0) {
                      ...
                      if (uniqueArray.length > 1) {
                        ...
                        if (currentPos + 1 < uniqueArray.length) {
                          ...
                          range.setTerminalBounds(
                            `<${majMinPatch}-${uniqueArray[currentPos + 1]}.0`
                          );
                        }
                      }
                    } else {
                      ...
                      const otherPreReleaseRelatives = _.filter(
                        otherSemver,
                        key => {
                          const newParsed = semver.parse(
                            this.valueToSemver(key)
                          );
                          return (
                            newParsed.prerelease.length > 0 &&
                            `${newParsed.major}.${newParsed.minor}.${
                      ...
                      if (
                        ...
                        if (remainingRealizations.length > 0) {
                          ...
                          if (
                            semver.parse(remainingRealizations[0])
                              .prerelease[0] === parsed.prerelease[0]
                          ) {
                            ...
                            if (parsed.prerelease.length !== 1) {
                              ...
                              range.setTerminalBounds(
                                `<${majMinPatch}-${otherNamed.sort()[0]}.0`
                              );
                            }
                          } else {
                            // different alpha
        ...
        addSemverRealizations(semverRealization) {
          if (semver.valid(semverRealization)) {
            ...
            throw new Error(
              `Semver realizations must be valid semver, ${semverRealization} is not a valid semver.`
            );
          }
        }
        ...
        getSemverRealizations() {
          return this.sortSemverishArray(
            _.uniq(this.converter.semverRealizations)
          );
        }
