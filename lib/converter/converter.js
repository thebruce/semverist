        ...
        Object.keys(tmpClasses).forEach((typeKey) => {
          Object.keys(tmpClasses[typeKey]).forEach((elementKey) => {
            ...
            semverishKeys.forEach((semverishKey, index, collection) => {
              ...
              && typeKey === 'attribute') {
                if (collection.length > index + 1) {
                  range.adjustTerminalBounds(
                    tmpClasses[typeKey][elementKey][collection[index + 1]].range.getSemverish()
                  );
                }
              }
              else if (
                  ...
                  if (preReleaseRelatives.length > 0) {
                    ...
                    if (
                      ...
                      if (remainingRealizations.length > 0) {
                        ...
                        if (semver.parse(
                          remainingRealizations[0]).prerelease[0]
                          ===
                          parsed.prerelease[0]
                        ) {
                          // Same alpha or less specific prerelease.
                          const otherNamed = _.without(_.map(remainingRealizations,
                          ...
                          if (parsed.prerelease.length !== 1) {
                            ...
                            range.setTerminalBounds(
                              `<${majMinPatch}-${parsed.prerelease[0]}.${parsed.prerelease[1] + 1}` // eslint-disable-line max-len
                            );
                          }
                          ...
                          // different alpha
                          range.setTerminalBounds(
                            `<${majMinPatch}-${semver.parse(remainingRealizations[0]).prerelease[0]}.0` // eslint-disable-line max-len
                          );
                        }
                      }
                    }
              ...
              range.setRange();
              range.setSemveristRange();
              const tmpSemveristRange = range.getSemveristRange();
              // Check to see if we should merge in original extensions.
              if (_.get(
                this.getSemveristConfig(),
                'converter.converterType',
                'default'
              ) !== 'default') {
                const semverishValue = tmpSemveristRange.semverishValue;
                let extensions = {};
                if (this.getOriginalExtensions) extensions = this.getOriginalExtensions();
                if (_.has(extensions[semverishValue], elementKey)) {
                  tmpSemveristRange.extension = extensions[semverishValue][elementKey];
                }
              }
