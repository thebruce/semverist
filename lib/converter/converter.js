...
module.exports = function converterFactory(superNameSpace, pluginName) {
  ...
  return setUp.then(
    extendedClass => class converter extends extendedClass {
          ...
          Object.keys(tmpClasses[typeKey]).forEach((elementKey) => {
            ...
            semverishKeys.forEach((semverishKey, index, collection) => {
              ...
              && typeKey === 'attribute') {
                ...
                if (!this.adjacentTerminalSetter(adjacent)) {
      ...
      adjacentTerminalSetter(adjacent) {
        const tmpAdjacent = _.clone(adjacent);
        if (tmpAdjacent.length > 0) {
          this.setTerminalBounds(this.sortSemverishArray(tmpAdjacent).shift());
          return true;
        }
        return false;
      }
