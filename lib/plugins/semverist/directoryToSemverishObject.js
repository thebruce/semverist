...
module.exports = superclass => class extends superclass {
  ...
  setSemverishObject(semverishDirectoryPath) {
    ...
    directories.forEach((pather) => {
      ...
      _.setWith(originalExtensions, test, {}, {});
    });
    ...
    files.forEach((pather) => {
      ...
      // Read in file.
      const fileContents = this.getFileContents(pather.path);
      _.setWith(semverishObject, `${testLocation}.${testName}`, fileContents, {});
      _.setWith(originalExtensions, `${testLocation}.${testName}`, path.extname(pathSuffix), {});
    });
    this.semverishObject = semverishObject;
    this.setOriginalExtensions(originalExtensions);
  ...
  getFileContents(pather) {
    ...
    if (
      _.has(this.getSemveristConfig(), 'converter.encoding')
      ) tmpEncoding = this.getSemveristConfig().converter.encoding;
    return fs.readFileSync(pather, {encoding: tmpEncoding});
  }

  /**
   * Sets the extension for the src file at the same path
   *   as its semverishObject.
   *
   * @param {Object} extensions - An object with semverish keys from the
   *   src with file extensions values.
   */
  setOriginalExtensions(extensions) {
    this.originalExtensions = extensions;
  }

  /**
   * Returns the original extensions object.
   *
   * @returns {Object} - An extensions object.
   */
  getOriginalExtensions() {
    return this.originalExtensions;
  }
