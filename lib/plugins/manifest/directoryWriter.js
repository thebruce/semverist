...
module.exports = superclass => class extends superclass {
  ...
  directoryWriter(level) {
    ...
    semveristAtLevel.forEach((key) => {
      ...
      // (for roots) just key.
      try {
        if ((level !== 'root' && toSemver(key)) || (toSemver(`${levelPrep}${key}`))) {
          // This worked so we know we have a directory. Create directory.
          this.makeSemverDirectory(`${levelPrep}${key}`);
          // And keep on digging:
          this.directoryWriter(`${levelPrep}${key}`);
        }
      }
      catch (error) {
        // Attempt to catch prereleases.
        if (semver.valid(`${levelPrep}${key}`) !== null) {
          // Check to see the length of prerelease.
          const parsed = semverUtils.parse(`${levelPrep}${key}`);
          if (parsed.release && this.splitStringNoEmpties(parsed.release).length < 3) {
            // The length of 1 or 2 is fine, i.e. 0-alpha.0
            this.makeSemverDirectory(`${levelPrep}${key}`);
            this.directoryWriter(`${levelPrep}${key}`);
          }
          else {
            // Too long i.e. 0-alpha.0.peabody
