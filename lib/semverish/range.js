  ...
  return setUp.then(extendedClass => class range extends extendedClass {
    ...
    calculateImpliedTerminalBounds(semverishValue) {
      ...
      if (
        ...
        this.getSemveristConfig().semveristBehaviors.inheritence === 'semverImplied'
          ||
          (
            this.getOptions().inheritence === 'lazySemverist'
            && this.getSemveristElementType() !== 'attribute'
          )
        ) {
        ...
        // inheritence is null, values are what they are.
        calculatedTerminal = tmpSemverEarly;
      }
    ...
    setRange() {
      ...
      if (!this.getRange()) {
        ...
        tmpRange = this.getTerminalBounds();
      }
      this.range = tmpRange;
    }
     ...
     */
    makeInheritenceRangeFromSemverish(earlySemverish, exceptions, lowerBounds, useTerminal) {
      ...
      if (useTerminal) tmpSemverLater = this.getTerminalBounds();
      ...
      if (_.has(this.constructor.parseSemver(tmpSemverEarly), 'release')) {
        ...
        if (tmpSemverLater) {
          ...
          if (semver.satisfies(hierarchicalEnd, tmpSemverLater)) {
    ...
    makeLazySemveristRangeFromSemverish(
      ...
      if (setTerminal) terminalBoundSetter = true;
      if (exceptions) tmpExceptions = exceptions;
      ...
      if (semverishLevel === 'prerelease' || semverishLevel === 'build') {
        ...
        if (!prereleaseForwards) {
          ...
          // next patch version.
          if (terminalBoundSetter) this.terminalBounds = `<${nextPatchFromEarlierSemverish}`;
          return tmpRange;
        }
      ...
      if (tmpRange === `>=${terminalSemverRange}`) {
        return semver.validRange(
          `${tmpRange} ${this.calculateImpliedTerminalBounds(terminalSemverRange)}`
        );
      }
      return semver.validRange(`${tmpRange} ${terminalSemverRange}`);
    }
    ...
    makeExceptionRange() {
      if (this.getAdjustedExceptions().length) {
        ...
        if (this.useSemverImplied()) {
          ...
          // Our first item
          adjacencies.push({
            open: _.head(this.getAdjustedExceptions()),
            close: _.head(this.getAdjustedExceptions())
          });

          _.tail(this.getAdjustedExceptions()).forEach((exception) => {
            if (
          ...
          adjacencies.forEach((item) => {
            if (item.open === item.close) {
              ...
              if (
                ...
                if ((this.determineSemverishLevel(this.getSemverish()) === 'major' ||
                this.determineSemverishLevel(this.getSemverish()) === 'minor' ||
                this.determineSemverishLevel(
                  this.getSemverish()) === 'patch')
                  && this.determineSemverishLevel(item.open) === 'patch'
                ) {
                  rangePrep[0].operator = '<';
                  rangePrep.push(
                    semverUtils.parse(
