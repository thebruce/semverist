...
module.exports = function rangeFactory(superNameSpace, pluginName) {
  ...
  return setUp.then(extendedClass => class range extends extendedClass {
    ...
    makeInheritenceRangeFromSemverish(earlySemverish, laterSemverish) {
      ...
      if (_.has(this.constructor.parseSemver(tmpSemverEarly), 'release')) {
        ...
        if (tmpSemverLater) {
          ...
          // If we have a prerelease we want to push up until the patch version.
            return this.constructor.rangeMaker(tmpSemverEarly, '>=', tmpSemverLater, '<');
          }
        ...
        return this.constructor.rangeMaker(tmpSemverEarly, '>=', hierarchicalEnd, '<');
      }
      ...
      return this.constructor.rangeMaker(tmpSemverEarly, '>=', tmpRangeTop, '<');
    }
     ...
     * @returns  {string} - a valid semver range string.
     */
    static rangeMaker(firstValue, firstOperator, secondValue, secondOperator) {
      return semver.validRange(`${firstOperator}${firstValue} ${secondOperator}${secondValue}`);
    }
          ...
          adjacencies.forEach((item) => {
            if (item.open === item.close) {
              ...
              if (this.getExceptions().length === 1
                ||
                adjacencies.indexOf(item) !== adjacencies.length - 1) {
                rangePrep[0].operator = '<';
              }
