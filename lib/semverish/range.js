...
module.exports = function rangeFactory(superNameSpace, pluginName, molotovConfig = molotovDefaults) { // eslint-disable-line max-len
  ...
  return Promise.resolve(setUp).then(extendedClass => class range extends extendedClass {
    /**
     *
     *
    ...
    init(configs, semverish) {
      ...
      if (
        ...
        this.getSemveristConfig().semveristBehaviors.inheritence !== 'lazySemverist'
      ) {
      ...
      if (semverish) {
        ...
        this.setOptions();
        this.setTerminalBounds(this.calculateImpliedTerminalBounds(this.getSemverish()));
    ...
    calculateImpliedTerminalBounds(semverishValue) {
      ...
      if (
        ...
        calculatedTerminal = this.constructor.validateRange(`<${tmpTerminal}`);
      }
      else {
        ...
        if ((pathArray.length === 3 && !_.has(parsed, 'release')) || (_.has(parsed, 'build'))) {
          calculatedTerminal = semver.validRange(this.valueToSemver(semverishValue));
        }
          ...
          if (this.getTerminalBounds()) {
            ...
            calculatedTerminal = this.constructor.validateRange(`<${hierarchicalEnd}`);
          }
        }
        else {
          ...
          calculatedTerminal = this.constructor.validateRange(`<${this.valueToSemver(tmpTerminal)}`); // eslint-disable-line max-len
        }
    ...
    adjustTerminalBounds(semverish) {
      ...
      if ((semverish !== this.getSemverish()
        && semver.satisfies(this.valueToSemver(semverish), this.getTerminalBounds())
        && semver.satisfies(
      ...
      (
        ...
        this.terminalBounds = this.constructor.validateRange(`<${this.valueToSemver(semverish)}`);
      }
    }
    ...
    setRange() {
      ...
      if (!this.getRange()) {
        ...
        // on our config.
        tmpRange = this.pathToRange(this.getSemverish(), this.getOptions(), this.getLowerBounds());
      }
      else if (
        this.getSemveristConfig().semveristBehaviors.inheritence === 'lazySemverist'
        && this.getSemveristElementType() === 'attribute'
    ...
    pathToRange(pather, options) {
      ...
      if (this.useSemverImplied(options)) {
        ...
        return this.makeInheritenceRangeFromSemverish(pather, tmpExceptions, this.getLowerBounds());
      }
     ...
     *    a semver range.
     */
    makeInheritenceRangeFromSemverish(earlySemverish, exceptions, lowerBounds) {
      ...
      const tmpRangeTop = this.calculateImpliedTerminalBounds(earlySemverish);
      ...
      if (_.has(
        this.constructor.parseSemver(tmpSemverEarly),
        'release'
      )
          &&
    ...
    static rangeMaker(firstValue, firstOperator, secondValue, secondOperator, exception) {
      ...
      return semver.validRange(`${firstOperator}${firstValue} ${tmpException} ${secondItem}`);
    }
    ...
    adjustLowerBounds(semverish) {
      ...
      const childMakesForAdjustment = _.find(exceptions, ((value) => {
        const tmpAnalysis = this.analyzeSemverishAdjacency(tmpSemverish, value);
      ...
      if (childMakesForAdjustment) {
        ...
        const semverishLevel = this.determineSemverishLevel(childMakesForAdjustment);
        // Tmp Range top is the inheritence based termination of this range.
        const newLowerSemverish = this.despecify(
          semver.inc(this.valueToSemver(childMakesForAdjustment), semverishLevel),
          this.constructor.getSemverLevelIndex(semverishLevel) + 1
        );
    ...
    getAdjustedExceptions() {
      ...
      return this.sortSemverishArray(_.difference(this.getExceptions(), this.adjustedExceptions));
    }
    ...
    addException(semverishValue) {
      ...
      else if (this.getOptions().inheritence !== null
      ...
      && (
        this.constructor.getSemverLevelIndex(this.determineSemverishLevel(semverishValue))
          <= this.constructor.getSemverLevelIndex(this.determineSemverishLevel(this.getExceptions()[0])) // eslint-disable-line max-len
      )) {
    ...
    makeExceptionRange() {
      if (this.getAdjustedExceptions().length) {
        ...
        if (this.useSemverImplied()) {
          ...
          const adjacencies = this.createAdjacencyArray(this.getAdjustedExceptions());
          ...
          adjacencies.forEach((item) => {
            if (item.open === item.close) {
              ...
              const rangePrep = semverUtils.parseRange(this.makeInheritenceRangeFromSemverish(item.open)); // eslint-disable-line max-len
              ...
              if (
                ...
                if (this.testFinalItemAgainstRangeSemverish(this.getSemverish(), item.open)) {
                  rangePrep[0].operator = '<';
                  rangePrep.push(semverUtils.parse(semver.inc(
                    this.valueToSemver(semverUtils.stringify(rangePrep[0])),
                    'patch'
                  )));
                  rangePrep[1].operator = '>=';
                }
              ...
              tmpRange = `${tmpRange} ${this.constructor.rangeMaker(item.open, '<', item.close, '>')}`; // eslint-disable-line max-len
            }
          }, this);
