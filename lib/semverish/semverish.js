...
module.exports = function semverishFactory(superNameSpace, pluginName, molotovConfig = molotovDefaults) { // eslint-disable-line max-len
  ...
  return Promise.resolve(setUp).then(extendedClass => class semverish extends extendedClass {
    /**
    ...
    extractSemverishFromPath(semverishValue) {
      try {
        ...
        if (tmpArray.length > 1) {
          ...
          let element = tmpPath.replace(`${replacer}.`, '')
