...
module.exports = function semverishFactory(
  superNameSpace,
  pluginName,
  ...
  return Promise.resolve(setUp).then(
    extendedClass =>
      class semverish extends extendedClass {
        /**
        ...
        setSemveristElement(semverishValue) {
          const tmpElement = this.extractSemveristElementFromPath(
            semverishValue
          );
         ...
         *
         * @param {string} semverishValue - A path converted to a semverish value
         *   i.e. x[.y][.z][.a]
         *
        ...
        extractSemverishFromPath(semverishValue) {
          try {
            ...
            if (tmpArray.length > 1) {
              ...
              if (element !== '' && !_.isFinite(Number(element))) {
                ...
                if (elementArray.length > 1) {
                  ...
                  element.indexOf('0') === 0 &&
                  !_.isFinite(Number(elementArray[0]))
                ) {
                ...
                if (tmpPath.lastIndexOf(element) > -1) {
                  ...
                  throw new Error(
                    'There was a problem with your semverish path and the regex pattern used to identify your prereleases.'
                  );
                }
              }
            }
            ...
            throw new Error(
              `The semverish value must be able to be converted to a semver value. ${error.message}`
            );
          }
        }
         ...
         *   semverish path.
         *
         * @param {string} pather - A semverish path with an attribute attached.
         *
