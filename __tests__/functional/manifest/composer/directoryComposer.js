...
describe('Directory Composer', () => {
  ...
  test('write composition.', () => {
    ...
    const mkDirAsPromise = filePath =>
      new Promise((resolve, reject) =>
        temp.mkdir(filePath, (err, data) => (err ? reject(err) : resolve(data)))
      );
    ...
    return mkDirAsPromise('semverist')
      .then(dirPath => {
        _.set(
          tmpConfig,
          'semverImpliedOrchestraDirectory.composer.destination',
          dirPath
        );
        _.set(
          tmpConfig,
          'semverImpliedOrchestraDirectory.callPath',
          path.join(__dirname, './../../../../')
        );
      ...
      .then(schoen =>
        fs.readJsonSync(
          `${schoen.getCompositionDestination()}/1/1/1/clarinet.json`
        )
      )
      .then(obj =>
        expect(obj).toEqual({
